...
if [ $TRIMMING == "YES" ]; then
	mkdir -p $OUTPUT_DIR/QC/trimmomatic
 	if [ "$USE_SGE" = "1" ]; then
 		TRIMMOMATIC=$( qsub $SGE_ARGS  -pe openmp $THREADS -t 1-$sample_number -N $JOBNAME.TRIMMOMATIC $TRIMMING_CMD)
...
if [ "$USE_SGE" = "1" ]; then
	FASTQC_PRE=$( qsub $SGE_ARGS -pe openmp $THREADS -t 1-$sample_number -N $JOBNAME.FASTQ_PRE $FASTQC_PRETRIMMING)
    jobid_fastqc_pre=$( echo $FASTQC_PRE | cut -d ' ' -f3 | cut -d '.' -f1 )
	echo -e "FASTQC_PRE:$jobid_fastqc_pre\n" >> $OUTPUT_DIR/logs/jobids.txt
	if [ $TRIMMING == "YES" ]; then
		FASTQC_ARGS="${SGE_ARGS} -pe openmp $THREADS -hold_jid $jobid_trimmomatic"
