...
if (( $sample_number >= 3 )); then
	...
	BEDFILTER_CMD="$SCRIPTS_DIR/bed_filter.sh $VCF_FIL_NAME $EXOME_ENRICHMENT $OUTPUT_DIR/variant_calling/variants_gatk/variants $VCFFIL $OUTPUT_DIR"
	ANNOTATION_CMD="$SCRIPTS_DIR/annotation.sh $VCF_FIL_NAME $VCFNOVO $VCFDOUBLEHIT 'vcf_merge.vcf' $OUTPUT_DIR/annotation $REF_PATH $PED_FILE $KGGSEQ_PATH $GATK_PATH $OUTPUT_DIR/variant_calling/variants_gatk/variants"
fi
